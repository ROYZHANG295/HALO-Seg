...
class SPPF(BaseModule):
    """Spatial Pyramid Pooling - Fast (SPPF) layer for PIDNet."""
    ...
    def __init__(self, 
        ...
        # 使用 ConvModule 自动添加 BN 和 ReLU
        self.cv1 = ConvModule(
            in_channels, c_, 1, stride=1, 
            norm_cfg=norm_cfg, act_cfg=act_cfg)
        self.cv2 = ConvModule(
            c_ * 4, out_channels, 1, stride=1, 
            norm_cfg=norm_cfg, act_cfg=act_cfg)
...
class PIDNetSPPF(BaseModule):
    ...
    def __init__(self,
        ...
        if num_stem_blocks == 2:
            ...
            # spp_module = PAPPM
            # ▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼ 修正点 2: 正确初始化 SPPF ▼▼▼▼▼▼▼▼▼▼▼▼▼▼▼
            # 不再动态赋值 spp_module，而是显式区分，因为参数签名不同
            self.spp = SPPF(
                in_channels=channels * 16,     # 输入: 1024 (若channels=64)
                out_channels=channels * 4,     # 输出: 256 (必须匹配 dfm 的输入)
                k=5,
                norm_cfg=norm_cfg,
                act_cfg=act_cfg
            )
