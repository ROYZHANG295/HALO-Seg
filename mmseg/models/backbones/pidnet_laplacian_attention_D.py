...
#########################################################################
# ######## [修改点 1] LaplacianAttention 类 (包含 Bias=-6 策略) ########
#########################################################################
class LaplacianAttention(nn.Module):
    ...
    def forward(self, x_raw, target_shape):
        ...
        # 🛠️ AMP 兼容性检查
        if x_gray.dtype != self.kernel.dtype:
            kernel = self.kernel.to(dtype=x_gray.dtype)
        else:
            kernel = self.kernel

        # 提取边缘
        edge = F.conv2d(x_gray, kernel, padding=1)
        edge = torch.abs(edge)
...
class PIDNetLaplacianAttentionD(BaseModule):
    """PIDNet backbone.
    ...
    def __init__(self,
        ...
        if num_stem_blocks == 2:
            ...
            # 🔥 动态获取 D 分支第一层的输出通道数
            # PIDNet-S (num_stem_blocks=2): channels
            d_out_channels = channels 
        else:
            self.d_branch_layers = nn.ModuleList([
            ...
            # 🔥 动态获取 D 分支第一层的输出通道数
            # PIDNet-M/L: channels * 2
            d_out_channels = channels * 2

        #########################################################################
        # ######## [修改点 2] 初始化 D 分支的注意力模块 ########
        #########################################################################
        # 放在 D 分支第一层之后
        self.d_branch_attention = LaplacianAttention(out_channels=d_out_channels)
        #########################################################################
    ...
    def init_weights(self):
        ...
        #########################################################################
        # ######## [修改点 3] 强制覆盖 D 分支注意力的初始化 ########
        #########################################################################
        # 必须在循环之后调用，防止被 Kaiming 覆盖
        if hasattr(self, 'd_branch_attention'):
            self.d_branch_attention.init_zero()
        #########################################################################
    ...
    def forward(self, x: Tensor) -> Union[Tensor, Tuple[Tensor]]:
        ...
        #########################################################################
        # ######## [修改点 5] 在 D 分支应用 Laplacian Attention ########
        #########################################################################
        # 1. 计算注意力 (输入 x_raw，对齐到 x_d 的尺寸)
        att_d = self.d_branch_attention(x_raw, x_d.shape[2:])
        
        # 2. 增强特征
        # 初始状态 Bias=-6 -> att_d ≈ 0.0025 -> x_d * 1.0025 (无感介入)
        x_d = x_d * (1 + att_d)
        #########################################################################
