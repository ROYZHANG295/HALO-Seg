...
class LaplacianAttention(nn.Module):
    ...
    def forward(self, feature_map):
        """
        feature_map: 当前阶段的特征图 (B, C, H, W)
        """
        # 1. 沿通道维度求均值，将多通道特征压缩为单通道“特征强度图”
        feat_gray = feature_map.mean(dim=1, keepdim=True) # (B, 1, H, W)
        
        # 2. 在特征图上提取高频边缘 (尺寸完全一致，无需插值！)
        edge = F.conv2d(feat_gray, self.kernel, padding=1)
        edge = torch.abs(edge)
        
        # 3. 🔥 核心维稳步骤：对边缘图进行归一化，防止数值爆炸
        B = edge.shape[0]
        edge_flat = edge.view(B, -1)
        edge_max = edge_flat.max(dim=1)[0].view(B, 1, 1, 1).clamp(min=1e-8)
        edge_norm = edge / edge_max # 归一化到 [0, 1]
...
class PIDNetLaplacianAttentionIZeroNorm(BaseModule):
    """PIDNet backbone.
    ...
    def forward(self, x: Tensor) -> Union[Tensor, Tuple[Tensor]]:
        ...
        # ######## [修改点 5] 计算并应用特征注意力 ########
        #########################################################################
        # 直接把 x_i 传进去提取特征级边缘，不需要 x_raw，也不需要传 target_shape
        att = self.edge_attention(x_i) 
        
        # 乘法增强 (Residual Attention)
        x_i = x_i * (1 + att)
        #########################################################################
