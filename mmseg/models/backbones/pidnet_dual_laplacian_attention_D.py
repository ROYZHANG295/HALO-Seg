...
# =====================================================================
# [新增模块 1] 专供 D 分支：高频残差锐化
# =====================================================================
class HighFreqSharpening(nn.Module):
    """
    作用于 D 分支：直接将拉普拉斯高频响应作为残差注入，显式锐化物理边界。
    数学等价于传统 CV 中的 Laplacian Sharpening。
    gamma 零初始化，插入初期等价于 Identity，不影响预训练权重。
    """
    def __init__(self, channels):
        super().__init__()
        self.channels = channels

        # 中心为正，四周为负：保证 x + gamma * H 是锐化方向
        kernel = torch.tensor([[  0., -1.,  0.],
                               [ -1.,  4., -1.],
                               [  0., -1.,  0.]], dtype=torch.float32)
        kernel = kernel.view(1, 1, 3, 3).repeat(channels, 1, 1, 1)

        # register_buffer：自动处理 GPU 迁移和模型保存，不参与梯度更新
    ...
    def forward(self, x: Tensor) -> Tensor:
        # .to(x.dtype) 适配 AMP 混合精度训练 (FP16)
        weight = self.laplacian_kernel.to(x.dtype)
        # 分组深度卷积：每个通道独立提取高频响应
        high_freq = F.conv2d(x, weight, padding=1, groups=self.channels)
        # 残差注入：gamma=0 时完全等价于原始特征
        return x + self.gamma * high_freq


# =====================================================================
# [新增模块 2] 专供 I 分支：低频残差平滑
# =====================================================================
class LowFreqSmoothing(nn.Module):
    """
    作用于 I 分支：促使特征向局部均值逼近，滤除高频纹理噪声，
    增强语义区域内部的一致性。
    gamma 零初始化，插入初期等价于 Identity，不影响预训练权重。
    """
    def __init__(self, channels):
        ...
        # 3x3 均值平滑核（低通滤波器，所有权重相等且和为 1）
        kernel = torch.full((1, 1, 3, 3), 1/9., dtype=torch.float32)
    ...
    def forward(self, x: Tensor) -> Tensor:
        # .to(x.dtype) 适配 AMP 混合精度训练 (FP16)
        weight = self.smooth_kernel.to(x.dtype)
        # 分组深度卷积：每个通道独立提取低频平滑值
        smooth_feat = F.conv2d(x, weight, padding=1, groups=self.channels)
        # 残差注入：smooth_feat - x 是"需要补充的低频分量"
        # gamma=0 时完全等价于原始特征；gamma=1 时完全变为平滑图
        return x + self.gamma * (smooth_feat - x)
...
class PIDNetDualFreqD(BaseModule):
    """PIDNet backbone.
    ...
    def __init__(self,
        ...
        # =====================================================================
        # [核心修改] 实例化双频残差滤波模块
        # 插入位置：Stage 3 之后（分辨率 1/8，是黄金位置）
        #
        # I 分支 Stage 3 输出通道：channels * 4 = 256（默认 channels=64）
        # D 分支 Stage 3 输出通道：
        #   num_stem_blocks == 2 时 -> channels     = 64
        #   num_stem_blocks != 2 时 -> channels * 2 = 128
        # =====================================================================
        i_branch_channels = channels * 4
        d_branch_channels = channels if num_stem_blocks == 2 else channels * 2

        self.i_branch_lfcs = LowFreqSmoothing(channels=i_branch_channels)
        self.d_branch_hfbs = HighFreqSharpening(channels=d_branch_channels)
    ...
    def forward(self, x: Tensor) -> Union[Tensor, Tuple[Tensor]]:
        ...
        # =====================================================================
        # [核心改动 1] I 分支 Stage 3 后：低频残差平滑
        # 目的：消除语义区域内部的高频纹理噪声（减少"内部碎块"）
        # gamma 初始为 0，完全不影响训练起步阶段
        # =====================================================================
        # x_i = self.i_branch_lfcs(x_i)

        x_p = self.p_branch_layers[0](x)
        x_d = self.d_branch_layers[0](x)

        # =====================================================================
        # [核心改动 2] D 分支 Stage 3 后：高频残差锐化
        # 目的：显式强化物理边界的空间突变（提升 Boundary IoU）
        # gamma 初始为 0，完全不影响训练起步阶段
        # =====================================================================
        x_d = self.d_branch_hfbs(x_d)
